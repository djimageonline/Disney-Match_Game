...
function createNewCardTest() {
  ...
  let step1Hint = constructHintMessage(
    1,
    `Try starting by calling document.createElement — this will create and return a new DOM element (the card) that you can store in a variable.`,
    RESOURCES.createElement
  );

  // Check step 2 heuristics.
  let classListAddPresentSpec = functionSpec(createNewCard).contains("classList.add").andThen("card");
  let classNameEqualsPresentSpec = functionSpec(appendNewCard).contains("className").andThen("card");
  let passedStep2Heuristics =
    checkFunctionSpec(classListAddPresentSpec) || checkFunctionSpec(classNameEqualsPresentSpec);
  let step2Hint = constructHintMessage(
    2,
    `Remember that you can access and manipulate an element's classes using cardElement.classList.  Try checking out the docs for classList.add().`,
  ...
  let step3Hint = constructHintMessage(
    3,
    `Try using innerHTML on your card element to define the two new divs with JavaScript template literals (using the backticks). You can define the divs with the same HTML syntax you use in your index.html!`,
    RESOURCES.innerHTML
  );
  ...
  if (!(card instanceof Element)) {
    console.log(
      constructErrorMessage(TEST_NAME, 4, `This function should return a DOM element, but that's not what we got.`)
    );
    return false;
  }
  ...
  if (card.children.length !== 2) {
    console.log(
      constructErrorMessage(
        TEST_NAME,
        3,
        `You should be adding exactly 2 children to the innerHTML of the card, but we see ${card.children.length}.`
      )
    );
    console.log("\nHere's what the HTML looks like:\n" + serialize(card) + "\n");
    console.log(step3Hint);
  ...
  if (!cardDown.classList.contains("card-down")) {
    console.log(
      constructErrorMessage(TEST_NAME, 3, `The first child of the card should be a div with the class 'card-down'.`)
    );
    console.log("\nHere's what the HTML looks like:\n" + serialize(card) + "\n");
    console.log(step3Hint);
    return false;
  ...
  if (!cardUp.classList.contains("card-up")) {
    console.log(
      constructErrorMessage(TEST_NAME, 3, `The second child of the card should be a div with the class 'card-up'.`)
    );
    console.log("\nHere's what the HTML looks like:\n" + serialize(card) + "\n");
    console.log(step3Hint);
    return false;
...
function appendNewCardTest() {
  ...
  let step1Hint = constructHintMessage(
    1,
    `You should be starting by calling 'createNewCard' and storing the result in a variable.`,
    RESOURCES.createElement
  );
  ...
  if (returnValue !== card) {
    console.log(
      constructErrorMessage(
        TEST_NAME,
        3,
        `The return value of this function should be the card but that's not what we got.`
      )
    );
...
function shuffleCardImageClassesTest() {
  ...
  let step1Hint = constructHintMessage(
    1,
    `Try starting by declaring a variable that holds an array of strings in the form ["image-1", "image-1", "image-2", "image-2", "image-3", ... , "image-6", "image-6"]`,
    RESOURCES.arrays
  );

  let step2HintLoadUnderscore = constructHintMessage(
    2,
    `The CDN for underscore must be included in your script tags above all other scripts, like this: ${COLOR_CODE.cyan}<script src="https://cdn.jsdelivr.net/npm/underscore@1.13.1/underscore-umd-min.js"></script>${COLOR_CODE.reset}`
  );

  // Check step 3 heuristics.
  let underScoreAndShuffleUsed = functionSpec(shuffleCardImageClasses).contains("_").andThen(".shuffle");
  let passedStep3Heuristics = checkFunctionSpec(underScoreAndShuffleUsed);
  let step3Hint = constructHintMessage(
    3,
    `We did not find use of _.shuffle. Your return needs to use _.shuffle using your image classes as the argument.`,
    ...
    if (!passedStep1Heuristics) {
      ...
      console.log(
        constructErrorMessage(
          TEST_NAME,
          2,
          "JavaScript does not understand the _ (underscore) in your code, meaning that the underscore.js library was not properly loaded."
        )
      );
  ...
  let allGoodClassNames = imageArray1.every((el) => {
    return el.startsWith("image-") && el.length == 7 && el[6] >= 1 && el[6] <= 6;
  });
  ...
  if (allSame) {
    console.log(
      constructErrorMessage(
        TEST_NAME,
        2,
        ` Function should return a random array of class names but 4 comparison calls returned the same array.`
      )
    );
    console.log(
...
function createCardsTest() {
  ...
  // Check step 1 heuristics.
  let variableAndEmptyArrayPresentSpec = functionSpec(createCards).contains("let").or("var").or("const").andThen("[]");
  let passedStep1Heuristics = checkFunctionSpec(variableAndEmptyArrayPresentSpec);
  let step1Hint = constructHintMessage(
    1,
    `Try starting by creating a variable that holds an empty array: [].`,
    RESOURCES.arrays
  );

  // Check step 2 heuristics.
  let forLoopPresentSpec = functionSpec(createCards).contains("for").andThen("12").andThen("++");
  let passedStep2Heuristics = checkFunctionSpec(forLoopPresentSpec);
  let step2Hint = constructHintMessage(
    2,
    `Try checking that you have a for loop iterating 12 times.`,
    RESOURCES.forLoops
  );
  ...
  let step2aHint = constructHintMessage(
    "2a",
    `Try declaring a variable that stores the result of calling appendNewCard with the argument 'parentElement'.`
  );

  // Check step 2b heuristics.
  let classListAddPresentSpec = functionSpec(createCards).contains("classList.add").andThen("shuffledImageClasses");
  ...
  let cardObjectspec = functionSpec(createCards).contains("index").andThen("element").andThen("imageClass");
  let passedStep2cHeuristics = checkFunctionSpec(cardObjectspec);
  let step2cHint = constructHintMessage(
    "2c",
    `Try checking that your object has 3 "key-value" pairs. Something like:
    {
  ...
  if (parent.children[0].classList[1] == undefined) {
    console.log(
      constructErrorMessage(
        TEST_NAME,
        2,
        `The classList for your new element should include an image class. We tested for classes and got: ${parent.children[0].classList}`
      )
    );
    console.log(step2bHint);
    return false;
  ...
  if (!parent.children[0].classList[1].includes("image")) {
    console.log(
      constructErrorMessage(
        TEST_NAME,
        2,
        `The classList for your new element should include the "card" class and an image class. Your we tested for classes and got: ${parent.children[0].classList}`
      )
    );
    console.log(step2bHint);
    return false;
  ...
  if (typeof cards !== "object" || cards.length == "undefined") {
    console.error(
      constructErrorMessage(TEST_NAME, 3, `Should return an array of objects but returned ${typeof cards}.`)
    );

    console.log(
      constructHintMessage(
        3,
        `Remember to return the card object array you just made!  The return value of this function should be the array of 12 card objects but that's not what we got.`
      )
    );
    return false;
  }
  ...
  if (cards.length !== 12) {
    if (!passedStep2cHeuristics) {
      ...
      console.error(
        `${ERROR}should return 12 card objects as children but got ${parent.children.length}. Make sure you are pushing 12 cards onto your cards array!`
      );
    }
    return false;
  }
...
function doCardsMatchTest() {
  ...
  let step1Hint = constructHintMessage(
    1,
    `Try comparing the imageClass property on each card element. You can do this by returning the result of a camparison between two parameters (e.g. parm1.property == param2.property)."`,
    RESOURCES.equality
  );
  ...
  if (doCardsMatch(cardA, cardC)) {
    console.log(
      constructErrorMessage(
        TEST_NAME,
        1,
        `We supplied cards with different a imageClass for cardObject1 and cardObject2 but the function returned 'true'.`
      )
    );
    console.log(step1Hint);
    return false;
...
function incrementCounterTest() {
  ...
  let counterNameAtZero = functionSpec(incrementCounter).contains("if").andThen("counters").andThen("undefined");
  ...
  let step2Hint = constructHintMessage(
    2,
    `Make sure you're adding 1 to the counters[counterName]. The simplest way to achieve that is with the JavaScript increment operator`,
    RESOURCES.increment
  );

  let innerHtmlSet = functionSpec(incrementCounter).contains(".innerHTML").andThen("counters").andThen("[counterName]");
  let passedStep3Heuristics = checkFunctionSpec(innerHtmlSet);

  let step3Hint = constructHintMessage(
    3,
    `Try using innerHTML on parentElement and set it to the new counter value.`,
    RESOURCES.innerHTML
  );
    ...
    if (!passedStep1Heuristics) {
      ...
      console.log(
        constructErrorMessage(
          TEST_NAME,
          3,
          `This function should place '1' in parentElement (counter should increase to 1) after first call but got ${string1}`
        )
      );
      console.log(step3Hint);
    } else {
      console.log(
        constructErrorMessage(
          TEST_NAME,
          3,
          `This function should place '1' in parent (counter should increase to 1) after first call but got '${string1}'`
        )
      );
    }
    return false;
  ...
  if (value2 - value1 !== 1) {
    console.error(
      constructErrorMessage(
        TEST_NAME,
        2,
        `This function should increment value in parent by 1 but consecutive calls to the same counter but got '${string1}' and '${string2}'`
      )
    );
    console.log(step2Hint);
    return false;
  ...
  if (value3 !== 1) {
    console.error(
      constructErrorMessage(
        TEST_NAME,
        2,
        `This function should place '1' in parent after first call with new counterName but got '${string3}'`
      )
    );
    return false;
  }
